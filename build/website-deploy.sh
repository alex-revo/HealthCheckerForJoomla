...
#
# Usage: ./website-deploy.sh [--skip-indexnow] [--skip-docs]
...
# Step 1: Rebuild documentation (optional)
if [ "$SKIP_DOCS" = true ]; then
    echo -e "${BLUE}Skipping documentation build (--skip-docs)${NC}"
else
    echo -e "${YELLOW}Step 1: Rebuilding documentation...${NC}"
...
echo ""
echo -e "${YELLOW}Step 2: Fetching latest release information...${NC}"
cd "$PROJECT_ROOT"
...
if ! grep -qE '<link rel="stylesheet" href="/output\.css(\?[^"]*)?"\s*/?>' public/index.html; then
    echo -e "${RED}ERROR: HTML template missing CSS link!${NC}"
...
if grep -q '<style>.*tailwindcss' public/index.html; then
    echo -e "${YELLOW}WARNING: Found inline Tailwind CSS in HTML template!${NC}"
...
if [ $CSS_SIZE -lt 10000 ]; then
    echo -e "${RED}ERROR: CSS file is suspiciously small (${CSS_SIZE_KB}KB). Build may have failed.${NC}"
    exit 1
fi

if [ $CSS_SIZE -gt 200000 ]; then
    echo -e "${YELLOW}WARNING: CSS file is larger than expected (${CSS_SIZE_KB}KB).${NC}"
...
if [ -f "public/docs/sitemap.xml" ]; then
    ...
    echo -e "${YELLOW}Warning: docs/sitemap.xml not found. Run VitePress build first to generate it.${NC}"
fi
...
if [ -f "public/docs/llms.txt" ]; then
    ...
    echo -e "${YELLOW}Warning: docs/llms.txt not found. Run VitePress build first to generate it.${NC}"
fi
...
if [ -f "public/docs/llms-full.txt" ]; then
    ...
    echo -e "${YELLOW}Warning: docs/llms-full.txt not found.${NC}"
fi
...
if command -v npx &> /dev/null; then
    ...
    echo -e "${YELLOW}Warning: npx not found, skipping JS minification${NC}"
fi

# Process HTML: Update Schema.org and version display
...
if [ $HTML_SIZE -gt 500000 ]; then
    echo -e "${RED}ERROR: HTML file is too large (${HTML_SIZE_KB}KB)!${NC}"
...
if grep -qE '<link rel="stylesheet" href="/output\.css(\?[^"]*)?"\s*/?>' public/index.html; then
    ...
    echo -e "${RED}ERROR: CSS link not found in HTML!${NC}"
    exit 1
fi
...
if grep -q "tailwindcss v4" public/index.html; then
    ...
    echo -e "${RED}ERROR: Found inline Tailwind CSS in HTML (${CSS_COUNT} copies)!${NC}"
...
if [ -f "public/output.css" ]; then
    ...
    echo -e "${RED}ERROR: public/output.css not found!${NC}"
    exit 1
fi
...
if ! git diff --quiet website/public/docs/ 2>/dev/null; then
    CHANGES=1
fi
...
if [ $CHANGES -eq 1 ]; then
    echo ""
    echo -e "${YELLOW}Step 4: Committing website changes...${NC}"
...
- Rebuild and minify assets"
    ...
    echo ""
    echo -e "${BLUE}Step 4: No website changes to commit${NC}"
fi
...
# Step 6: Notify search engines via IndexNow
echo ""
if [ "$SKIP_INDEXNOW" = true ]; then
    echo -e "${BLUE}Step 6: Skipping IndexNow notification (--skip-indexnow)${NC}"
else
