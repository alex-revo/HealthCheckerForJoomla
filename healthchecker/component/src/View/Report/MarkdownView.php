...
class MarkdownView extends BaseHtmlView
{
    ...
    private function renderMarkdownReport(
        ...
        $lines[] = '';
        $lines[] = Text::sprintf('COM_HEALTHCHECKER_REPORT_GENERATED_ON', $reportDate) . ' | Joomla ' . $joomlaVersion;
        $lines[] = '';

        // Summary table
        $lines[] = '## ' . Text::_('COM_HEALTHCHECKER_REPORT_SUMMARY');
        $lines[] = '';
        $lines[] = '| Status | Count |';
        $lines[] = '|--------|------:|';
        ...
        $lines[] = '| **' . Text::_('COM_HEALTHCHECKER_REPORT_TOTAL') . '** | **' . $totalCount . '** |';
        $lines[] = '';
        ...
        foreach ($results as $categorySlug => $categoryResults) {
            ...
            foreach ($categoryResults as $categoryResult) {
                ...
                if ($categoryResult->docsUrl !== null) {
                    $description .= ($description !== '' ? ' ' : '') . '[' . Text::_('COM_HEALTHCHECKER_REPORT_DOCS') . '](' . $categoryResult->docsUrl . ')';
                }
        ...
        $lines[] = '---';
        $lines[] = '';
        $footer = Text::_('COM_HEALTHCHECKER_REPORT_GENERATED_BY') . ' [Health Checker for Joomla](https://www.joomlahealthchecker.com/) | ' . Text::_('COM_HEALTHCHECKER_REPORT_FREE_EXTENSION') . ' [mySites.guru](https://mysites.guru)';
        ...
        $filteredProviders = array_filter(
            $thirdPartyProviders,
            fn(\MySitesGuru\HealthChecker\Component\Administrator\Provider\ProviderMetadata $providerMetadata): bool => isset($presentProviders[$providerMetadata->slug]),
        );
        ...
        if ($filteredProviders !== []) {
            ...
            foreach ($filteredProviders as $filteredProvider) {
                if ($filteredProvider->url !== null) {
                    $pluginLinks[] = '[' . $filteredProvider->name . '](' . $filteredProvider->url . ')';
                } else {
                    $pluginLinks[] = $filteredProvider->name;
                }
            }

            $footer .= ' | ' . Text::_('COM_HEALTHCHECKER_REPORT_COMMUNITY_PLUGINS') . ' ' . implode(
                ', ',
                $pluginLinks,
            );
        }
